...
. requiredDuringSchedulinglgnoredDuringExecution: Must
...
Benefits: Auditable deployments, faster disaster recovery, improved
...
kubectl get pods -[ <service-selector>: Verify that Pods with the correct labels
...
o Answer: A headless Service does not have a ClusterlP. Instead of acting
...
Answer: CRI is a plugin interface that enables Kubernetes to use different
...
Debugging nodes: Create a debug Pod on a specific node to inspect its
filesystem or processes.

Running network diagnostics: Use tools like tcpdump inside a debug
container.
...
Troubleshooting running Pods: Attach a debugger or run diagnostic tools
without restarting the main application container.
